...
```
```
Below shows a plot of our cross validated error for increasing levels of our complexity parameter.  We can see that it bottoms out very quickly, so a model with the parameter that minimizes this error will still be quite simple.  This is opposed to choosing the simplest model within one standard error of the minimum. Doing this would likely lead to a model that was too simple, leading our model to not split at all.  In this case, it seems we can afford to choose the parameter that minimizes cross-validated error.

```{r =FALSE, error=FALSE, message=FALSE, warning=FALSE}
# building the model:

dengue_CART = rpart(total_cases ~ ., data = dengue_train, control = rpart.control(cp = .00001))
...
rpart.plot(dengue_CART, type = 4, digits = 2, tweak = 2, fallen.leaves = TRUE, extra = 1)
